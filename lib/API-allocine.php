...
const UrlSearch = 'http://api.allocine.fr/rest/v3/search?';
const UrlMovie = 'http://api.allocine.fr/rest/v3/movie?';
const UrlSerie = 'http://api.allocine.fr/rest/v3/tvseries?';
const UrlSeason = 'http://api.allocine.fr/rest/v3/season?';
const UrlEpisode = 'http://api.allocine.fr/rest/v3/episode?';
...
public function movieInfos($id){
		$this->url = $this::UrlMovie.'partner='.$this->Options['partner'].'&code='.$id.'&profile='.$this->Options['profile'].'&format='.$this->Options['format'];
...
public function seasonInfos($id){
	$this->url = $this::UrlSeason.'code='.$id.'&partner='.$this->Options['partner'].'&format=json&profile=large';
	$this->content = file_get_contents($this->url);
	$this->data = json_decode($this->content,true);
...
public function episodeInfos($id){
	$this->url = $this::UrlEpisode.'code='.$id.'&partner='.$this->Options['partner'].'&format=json&profile=large';
	$this->content = file_get_contents($this->url);
	$this->data = json_decode($this->content, true);
	$this->result = array(
...
public function serieInfos($id){
	$this->url = $this::UrlSerie.'code='.$id.'&partner='.$this->Options['partner'].'&format=json&profile=large';
		//echo $this->url;
		$this->content = file_get_contents($this->url);
		$this->data = json_decode($this->content,true);
...
public function movieMultipleSearch($keywords,$count) {
		$keywords = urlencode($keywords);
		$this->url = $this::UrlSearch.'q='.$keywords.'&partner='.$this->Options['partner'].'&filter=movie&format=json&count='.$count.'&profile='.$this->Options['profile'].'&version='.$this->Options['version'];
		$this->content = file_get_contents($this->url);
		$this->data = json_decode($this->content,true);
		$this->result = array();
			for ($i=0;$i<$count;$i++){
	...
	private function get_url($keywords,$serie=false){
		...
		$this->url = $this::UrlSearch.'q='.$keywords;
