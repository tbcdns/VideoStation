...
for ($i=0;$i<=(count($edir)-1);$i++) {
	...
	echo '<a href="?rep='.$redir.'">'.$edir[$i].'</a> / ';
	}
}
...
$urlSyno=$http.$_SERVER['SERVER_ADDR'].':'.$port.'/webman/login.cgi?username='.$user.'&passwd='.$pass;
...
if(mb_ereg($SERIES_DIR,$_GET['rep'])) return true;
else return false;
}
...
function login_check($login, $port){
	if($login){
	...
	if($_GET['action'] == 'login') login($_POST['user'],$_POST['pass'],$_POST['cookie'],$port);
}
...
function index_auto($dir,$HIDDEN_FILES,$ext){
	$sql = "SELECT * FROM movies WHERE dir='".$dir."'";
